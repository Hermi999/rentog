Feature: User views marketplace
  In order to see the latest activity in Rentog
  As a user
  I want see latest offers, requests and transactions on the marketplace page

  @javascript
  Scenario: Latest offers on the marketplace
    ...
    When I am on the marketplace
    Then I should see "car spare parts"
    And I should not see "bike"
    And I should see "saw"

  @javascript
  Scenario: Latest requests on the marketplace
    Given there are following users:
      | person |
      | kassi_testperson1 |
      | kassi_testperson2 |
  ...
  @javascript
  Scenario: User browses marketplace when there is no content
    Given there are following users:
       | person |
       | kassi_testperson1 |
    When I am on the marketplace
  ...
  @pending
  Scenario: Latest transactions on the marketplace
